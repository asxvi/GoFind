...
func (res *Stats) printStats(stats Stats) {
	fmt.Printf(`
  Total Count: %d
  File Count:  %d
  Dir Count:   %d
  Total Size:  %d bytes or %.2f gb
  `, stats.totCount, stats.fileCount, stats.dirCount, stats.totByteSize, float64(stats.totByteSize)/1000000000.00)
}
